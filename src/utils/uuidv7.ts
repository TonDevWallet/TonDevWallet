  ...
  static parse(uuid: string): UUID {
    ...
    switch (uuid.length) {
      case 32:
        ...
        break
      case 36:
        hex = /^([0-9a-f]{8})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{12})$/i
          .exec(uuid)
          ?.slice(1, 6)
          .join('')
        break
      case 38:
        ...
        break
      case 45:
        hex = /^urn:uuid:([0-9a-f]{8})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{12})$/i
          .exec(uuid)
          ?.slice(1, 6)
          .join('')
        break
...
export const uuidv7obj = (): UUID => {
  const gen = defaultGenerator || (defaultGenerator = new V7Generator())
  return gen.generate()
}
